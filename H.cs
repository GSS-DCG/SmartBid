...
public static class H //Helper class for reading properties from an XML file
{
    ...
    private static bool _EnviarMail(string[] email)
    {
        try
        {
    ...
    private static string[] _GetEmail(DataMaster dm)
    {
        string[] email = [];

        string kam = dm.GetInnerText("/dm/initData/kam");
        string createdBy = dm.GetInnerText("/dm/utils/rev_01/requestInfo/createdBy");

        if (kam != null && createdBy != null)
        {
            if (kam == createdBy) { email = [createdBy]; }

            else if (kam != createdBy) { email = [createdBy, kam]; }

            else { PrintLog(2, ThreadContext.CurrentThreadInfo.Value.User, "_GetEmail:", "Error: No se ha podido crear la lista correos"); }
        }
        else
        {
            PrintLog(2, ThreadContext.CurrentThreadInfo.Value.User, "_GetEmail:", "Error: Parametro Kam o CreatedBy en blanco");
        }
        return email;
    }
